...
/// The test cases we aim to support for the `out.yaml` test.
///
/// Entries are commented out for now; enable them gradually as
/// `out.yaml` output support grows.
#[rustfmt::skip]
pub(crate) const SUPPORTED_OUT_YAML_TEST: &[&str] = &[
...
fn yaml_test_suit_out_yaml() {
    ...
    for test_path in test_paths.into_iter() {
        ...
        // TODO: parse `in.yaml`, emit it back to YAML and compare the
        // result with `out.yaml`.
