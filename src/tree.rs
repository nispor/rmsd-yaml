    ...
    pub(crate) fn handle_node(
        ...
        if let Some(line) = self.scanner.peek_line() {
            ...
            if trimmed.starts_with("- ") || trimmed == "-" {
                ...
                self.handle_block_map(first_indent_count, rest_indent_count)?;
...
//            "zero-indented-sequences-in-explicit-mapping-keys",
        ...
        for (i, test_path) in test_paths.into_iter().enumerate() {
            ...
            let expected_events =
                read_file(&test_path.join(TEST_EVENT_FILE_NAME));
    ...
    fn run_test(input_yaml: &str, expected_events: &str, is_error: bool) {
